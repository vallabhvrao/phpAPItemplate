 ...
 */
class custom_encryption_keys_class {
    ...
    function makeKey($key1, $key2, $key3) {
...
function custom_encrypt_it($first_key, $second_key, $_source) {
    try {
        $e1 = new custom_encryption_keys_class(MCRYPT_BLOWFISH, MCRYPT_MODE_CBC);

        $third_key = "Hard coded Secret"; // Let this be hard coded now
        if ($e1->IsStringEmpty($first_key)) {
            return "set KEY1 environment variable\n";
        }
        if ($e1->IsStringEmpty($second_key)) {
            return "set KEY2 environment variable\n";
        }
        $key = $e1->makeKey($first_key, $second_key, $third_key);

        if ($e1->IsStringEmpty($_source)) {
            return "set ENCRYPT_THIS environment variable\n";
        }
        $encryptedData = $e1->encrypt($_source, $key);
        $enc_key = bin2hex($encryptedData);
        return '[{"status" : "success","data" : "' . $enc_key . '"}]';
//        $resp = '[{"status" : "success","data" : "' . $enc_key . '","key1":"' . $first_key . '","key2":"' . $second_key . '","key3":"' . $third_key . '","source":"' . $_source . '"}]';
//        echo $resp;
        return $resp;
